...
def get_piece(x, y, board):
    ...
    #Ensure that x and y are both integers using assert
    assert type(x) == type(y) == int, "Coordinates must be integers"

    #What does this do? --> length of board; number of rows
    N = len(board)
...
def have_lost(board):
    ...
    #return False - deleted to start step 3
...
def swap(board):
    ...
    #Check if a swap may occur on the board
